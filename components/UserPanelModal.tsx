...
export function UserPanelModal({ visible, onClose }: UserPanelModalProps) {
  ...
  const text = useMemo(
    () => ({
      ...
      companySwitch: isTr ? 'Firma GeÁi˛i' : 'Company Switch',
      selectedCompany: isTr ? 'SeÁili firma' : 'Selected company',
      showOtherCompanies: isTr ? 'Dier firmalar˝ gˆster' : 'Show other companies',
      hideOtherCompanies: isTr ? 'Firma listesini gizle' : 'Hide company list',
      noOtherCompany: isTr ? 'Ba˛ka bal˝ firma yok.' : 'No other linked companies.',
      companyInfo: isTr ? '›˛letme Bilgileri' : 'Business Details',
      accountSettings: isTr ? 'Hesap Ayarlar˝' : 'Account Settings',
      languageSettings: isTr ? 'Dil Ayarlar˝' : 'Language Settings',
      legalSupport: isTr ? 'Yasal ve Destek' : 'Legal and Support',
      companyName: isTr ? 'Firma ad˝' : 'Company name',
      ...
      password: isTr ? 'Yeni ˛ifre' : 'New password',
      passwordPlaceholder: isTr ? 'En az 6 karakter' : 'At least 6 characters',
      updatePassword: isTr ? 'ﬁifreyi G¸ncelle' : 'Update Password',
      updatingPassword: isTr ? 'G¸ncelleniyor...' : 'Updating...',
      currency: isTr ? 'Para Birimi' : 'Currency',
      theme: isTr ? 'Tema' : 'Theme',
      switchToLight: isTr ? 'AÁ˝k moda geÁ' : 'Switch to light mode',
      switchToDark: isTr ? 'Koyu moda geÁ' : 'Switch to dark mode',
      deleteHint: isTr ? 'Hesap silme i˛lemi geri al˝namaz.' : 'Account deletion cannot be undone.',
      deletionReason: isTr ? 'Silme nedeni' : 'Deletion reason',
      deletionPlaceholder: isTr ? '›sterseniz k˝sa bir not yazabilirsiniz.' : 'You can add a short note if you want.',
      deleting: isTr ? 'Hesap siliniyor...' : 'Deleting account...',
      deleteAccount: isTr ? 'Hesab˝ Kal˝c˝ Olarak Sil' : 'Delete Account Permanently',
      logout: isTr ? '«˝k˝˛ Yap' : 'Sign Out',
      version: isTr ? 'Versiyon' : 'Version',
      companyUpdated: isTr ? 'Firma bilgileri g¸ncellendi.' : 'Company details updated.',
      companySaveFailed: isTr ? 'Firma bilgileri kaydedilemedi.' : 'Could not save company details.',
      passwordUpdated: isTr ? 'ﬁifreniz g¸ncellendi.' : 'Your password has been updated.',
      passwordFailed: isTr ? 'ﬁifre dei˛tirilemedi.' : 'Could not update the password.',
      logoutFailed: isTr ? '«˝k˝˛ yap˝lamad˝.' : 'Could not sign out.',
      companyMissing: isTr ? 'D¸zenlenecek firma bulunamad˝.' : 'No company found to edit.',
      companyRequired: isTr ? 'Firma ad˝ bo˛ olamaz.' : 'Company name cannot be empty.',
      deleteConfirmTitle: isTr ? 'Hesap kal˝c˝ olarak silinsin mi?' : 'Delete account permanently?',
      deleteConfirmText: isTr
        ? 'Bu i˛lem geri al˝namaz. Hesab˝n˝z ve bal˝ veriler silinir veya anonimle˛tirilir.'
        : 'This action cannot be undone. Your account and related data will be deleted or anonymized.',
      cancel: isTr ? 'VazgeÁ' : 'Cancel',
      deleted: isTr ? 'Hesap Silindi' : 'Account Deleted',
      deletedText: isTr ? 'Hesab˝n˝z ba˛ar˝yla silindi.' : 'Your account has been deleted successfully.',
      legalLinks: [
        { label: isTr ? 'Gizlilik Politikas˝' : 'Privacy Policy', route: '/privacy-policy' as never },
        { label: isTr ? 'Destek Bilgileri' : 'Support Information', route: '/support' as never },
        { label: isTr ? 'Kullan˝m Ko˛ullar˝' : 'Terms of Service', route: '/terms-of-service' as never },
        { label: isTr ? 'KVKK Ayd˝nlatma Metni' : 'KVKK Notice', route: '/kvkk-notice' as never },
      ],
    }),
    [isTr]
  );
  ...
  const roleLabel = (role?: 'owner' | 'admin' | 'user') => {
    ...
    if (role === 'admin') return isTr ? 'Y√∂netici' : 'Admin';
    return isTr ? 'Kullanƒ±cƒ±' : 'User';
  };
